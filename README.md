...
This is a test project which compiles an Executable and Dynamic Link-Library into Alternate-Data-Streams (ADS) I have named `helloworld-DLL.ps1:.exe` and `helloworld-DLL.ps1:.dll`.  The Powershell script, when run, will create symbolic links to the streams and name them; `helloworld-app.exe` and `helloworld-DLL.dll`.  Files seem to be able to tell what other files are in the directory with it.  So you can't just run `helloworld-DLL.ps1:.exe` it will complain there is no `.dll` file even though its in data stream in the same file!  The files need to be next to each other in the same directory.  I do not know why.
